...
if (file.exists(paste0(out_dir,"/Meta-analysis"))) {
if (length(dot_go)>1) {print(dot_go[[2]])}}

if (file.exists(paste0(out_dir,"/Meta-analysis"))) {
if (length(dot_go)>2) {print(dot_go[[3]])}}

if (file.exists(paste0(out_dir,"/Meta-analysis"))) {
cnetp=list.files(path=paste0(out_dir,"/Meta-analysis/Gene_ontology"),
                            pattern = "cnetplot_GO_.*\\.pdf",
                            full.names = T)
cnet_go=list()
for (i in 1:length(cnetp)) {
  cnet_go=list.append(cnet_go,image_read_pdf(cnetp[i]))
}
print(cnet_go[[1]])}

if (file.exists(paste0(out_dir,"/Meta-analysis"))) {
if (length(cnet_go)>1) {print(cnet_go[[2]])}}

if (file.exists(paste0(out_dir,"/Meta-analysis"))) {
if (length(cnet_go)>2) {print(cnet_go[[3]])}
}
```
