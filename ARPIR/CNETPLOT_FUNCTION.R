...
setting.graph.attributes <- function(g, node.size=4,
                                     node.color="#B3B3B3",
                                     edege.width=1,
                                     edege.color="#8DA0CB") {
  V(g)$size <- node.size
...
get.col.scale <- function(foldChange, DE.foldChange=FALSE) {
  ...
  if (DE.foldChange) {
    ...
    col.down <- cscale(FC.down, seq_gradient_pal("dodgerblue3", "dodgerblue"))
    ## col.up <- cscale(FC.up, seq_gradient_pal("red", "darkred"))
    col.up <- cscale(FC.up, seq_gradient_pal("red", "red3"))
  } else {
    col.down <- cscale(FC.down, seq_gradient_pal("dodgerblue3", "gray88"))
    col.up <- cscale(FC.up, seq_gradient_pal("gray88", "red3"))
  }
...
netplot <- function(g,
  ...
  if (fixed){
    ...
    if (!is.null(foldChange)) {
      ...
      points(x, y, pch=15, col=col.legend, cex=0.8)
...
cnetplot_internal <- function(inputList,
  ...
  V(g)[1:lengthOfCategory]$color= "bisque"
